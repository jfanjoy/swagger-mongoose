...
var propertyMap = function (property) {
  switch (property.type) {
    ...
    case 'object':
      if ('format' in property && property.format === 'map') {
        return Map
      }
    default:
...
var processRef = function (property, objectName, props, key, required) {
  ...
  if (propType !== objectName) {
    ...
    let propValue
    if ('properties' in definitions[propType]) {
      propValue = getSchema(propType, definitions[propType]['properties'])
    } else {
      propValue = [getSchema(propType, definitions[propType])]
    }
    props[key] = propValue
  } else {
    // circular reference
...
module.exports.compile = function (spec, _extraDefinitions) {
  ...
  var schemas = {}
  var modelExtensionDir = path.resolve(process.cwd(), 'api/models')
  ...
  _.forEach(definitions, function (definition, key) {
    ...
    if (object && !excludedSchema) {
      ...
      var modelExtras = `${modelExtensionDir}/${key.toLowerCase()}`
      if (fs.existsSync(`${modelExtras}.js`)) {
        schema = require(modelExtras)(schema)
      }
      schemas[key] = schema
    }
  })
